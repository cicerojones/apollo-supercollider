...
#+RESULTS:
| Channel-one | 0 | 0 | 0 | 0 | 0 | 0 | 0 | 0 | Channel-two | 0 | 0 | 0 | 0 | 0 | 0 | 0 | 0 |
...
** use "reset-knob" and "swap-pad" to control "general-tgrains"
...
** create event-handlers for general-tgrains
...
#+BEGIN_SRC clojure :results silent
  (on-event [:midi :control-change]
            (reset-knob general-tgrains :amp 2 cc2-1 1 :scale 0.01)
            :amp-knob2)

  (on-event [:midi :control-change]
            (reset-knob general-tgrains :centerpos 1 cc1-1 1 :scale 0.5)
            :centerpos-knob1)
#+END_SRC
...
#+BEGIN_SRC clojure :results silent
  (on-event [:midi :note-on]
            (swap-pad general-tgrains :centerpos 38 pad3-1 inc 1)
            :centerpos-pad3-inc)

  (on-event [:midi :note-on]
            (swap-pad general-tgrains :centerpos 39 pad3-1 dec 1)
            :centerpos-pad4-dec)

  (on-event [:midi :note-on]
            (swap-pad general-tgrains :dur-numerator 36 pad1-1 inc 1)
            :durnumerator-pad1)

  (on-event [:midi :note-on]
            (swap-pad general-tgrains :dur-numerator 37 pad1-1 dec 1)
            :durnumerator-pad2)

(on-event [:midi :note-on]
            (swap-pad general-tgrains :trate 42 pad7-1 (fn [x] (+ 0.1 x)) 1)
            :trate-pad7-inc)

  (on-event [:midi :note-on]
            (swap-pad general-tgrains :trate 43 pad7-1 (fn [x] (- x 0.1)) 1)
            :trate-pad8-dec)
#+END_SRC
